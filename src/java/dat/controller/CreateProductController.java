    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (isMultiPart) {
                ...
                ProductDTO dto = new ProductDTO(productID, productName, price, quantity, fileName, description, "Active", category);
                
                request.setAttribute("PRODUCT", dto);
                if (productID.trim().isEmpty()) {
                    request.setAttribute("ERROR", "ID can not null");
                } else if (productName.trim().isEmpty()) {
                    request.setAttribute("ERROR", "Name of product can not null");
                } else if (quantity < 1) {
                    request.setAttribute("ERROR", "Quantity is cannot < 1 or null");
                } else if (price < 1) {
                    request.setAttribute("ERROR", "Price is cannot < 1 or null");
                } else if (category.equals("Category")) {
                    request.setAttribute("ERROR", "Please choice category");
                } else {
                    ProductDAO dao = new ProductDAO();
                    int result = dao.create(dto);
                    if (result != 1) {
                        request.setAttribute("ERROR", "The id already exists");
                    } else {
                        url = SUCCESS;
                    }
                }
