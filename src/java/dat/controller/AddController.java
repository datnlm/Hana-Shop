    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (user != null) {
                ...
                ProductDTO dto = new ProductDTO(productID, productName, price, 1, "", "", "active");
                if (cart == null) {
                    ...
                    for (ProductDTO p : cart.getCart().values()) {
                        if (dto.getProductID().equals(productID)) {
                            quantityCart = p.getQuantity();
                        }
                    }
                    int quantityStock = dao.getQuantity(productID);
                    if (quantityStock <= quantityCart) {
                        flag = false;
                        request.setAttribute("ERROR", "Sorry, you can only purchase up to " + quantityStock + " " + productName);
                    }
                }
                if (flag) {
                    cart.add(dto);
